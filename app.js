(function () {
  ...
  var customerDiscountRate = Number((catalog.pricing && catalog.pricing.customerDiscountRate) || 15);
  var priceSourceLabel = (catalog.pricing && catalog.pricing.priceColumn) || "Precio 4";
  ...
  function bindEvents() {
    ...
    dom.downloadExcel.addEventListener("click", function () {
      downloadExcel();
    });
  ...
  function renderStats() {
    ...
    dom.sourceLabel.textContent = "Inventario · " + priceSourceLabel + " · " + formatCatalogDate(catalog.generatedAt);
  ...
  function renderCategoryRail() {
    dom.categoryRail.innerHTML = categories.map(function (category) {
      ...
      return [
        ...
        '<span class="category-icon" aria-hidden="true">' + iconHtml(categoryIcon(category.id)) + "</span>",
  ...
  function renderBuildOverview() {
    ...
    dom.buildOverview.innerHTML = slots.map(function (categoryId) {
      ...
      return [
        '<div class="' + classes + '">',
        '<div class="build-slot-label"><span>' + iconHtml(categoryIcon(categoryId)) + " " + escapeHtml(category.title) + '</span><span>' + escapeHtml(category.short) + "</span></div>",
  ...
  function renderProductCard(product) {
    ...
    return [
      ...
      '<span class="brand-chip"><i class="fa-solid fa-tag" aria-hidden="true"></i>' + escapeHtml(product.brand || "Marca") + "</span>",
      '<span class="status-chip ' + compat.level + '">' + escapeHtml(statusText) + "</span>",
      "</div>",
      '<p class="product-name">' + escapeHtml(product.name) + "</p>",
      '<div class="product-meta">',
      '<span class="stock-chip"><i class="fa-solid fa-boxes-stacked" aria-hidden="true"></i>' + escapeHtml(String(product.stock)) + " disp.</span>",
      '<span class="product-code">UPC ' + escapeHtml(productUpc(product)) + "</span>",
      "</div>",
      '<div class="tag-list">' + tags + "</div>",
      '<div class="product-footer">',
      '<div class="price-block"><span>' + escapeHtml(priceSourceLabel) + '</span><div class="price">' + currency.format(product.price) + "</div></div>",
      '<div class="product-buttons">',
      '<button class="detail-button" type="button" data-detail="' + escapeHtml(product.id) + '" title="Ver ficha" aria-label="Ver ficha"><i class="fa-solid fa-circle-info" aria-hidden="true"></i></button>',
      '<button class="product-action" type="button" data-select="' + escapeHtml(product.id) + '">' + iconHtml(selected ? "fa-solid fa-check" : "fa-solid fa-plus") + escapeHtml(actionText) + "</button>",
      "</div>",
      "</div>",
      "</div>",
      "</article>"
  ...
  function renderQuote() {
    var analysis = analyzeBuild();
    var totals = calculateTotals(customerDiscountRate);
    ...
    dom.totalBox.innerHTML = [
      ...
      '<p class="pricing-note">' + escapeHtml(priceSourceLabel) + ' del inventario con descuento comercial único del ' + customerDiscountRate + '%.</p>',
      totalRow("Subtotal " + priceSourceLabel, currency.format(totals.subtotal)),
      totalRow("Descuento " + customerDiscountRate + "%", "-" + currency.format(totals.discount)),
      totalRow("Ensamble", "Gratis"),
      '<div class="discount-card featured single-discount">',
      '<span>Total final</span>',
      '<strong>' + currency.format(totals.total) + "</strong>",
      '<small>Ahorro aplicado ' + currency.format(totals.discount) + "</small>",
      "</div>",
      "</div>"
    ].join("");
  }
  ...
  function renderSelections() {
    ...
    dom.selectionList.innerHTML = entries.map(function (entry) {
      ...
      return [
        ...
        '<span>' + escapeHtml(category.title) + " · UPC " + escapeHtml(productUpc(entry.product)) + " · " + escapeHtml(priceSourceLabel) + " " + currency.format(entry.product.price) + "</span>",
        "</div>",
  ...
  function calculateTotals(discountRate) {
    ...
    var rate = Math.max(0, Number(discountRate == null ? customerDiscountRate : discountRate));
  ...
  function calculatePricingPlans() {
    return [calculateTotals(customerDiscountRate)];
  }
  ...
  function productUpc(product) {
    var value = String((product && (product.upc || product.barcode)) || "").trim();
    return value && value !== "0" ? value : "N/D";
  }

  function categoryIcon(categoryId) {
    var icons = {
      cpu: "fa-solid fa-microchip",
      motherboard: "fa-solid fa-diagram-project",
      ram: "fa-solid fa-memory",
      storage: "fa-solid fa-hard-drive",
      gpu: "fa-solid fa-display",
      case: "fa-solid fa-computer",
      psu: "fa-solid fa-plug-circle-bolt",
      ...
      accessory: "fa-solid fa-screwdriver-wrench"
    };
    return icons[categoryId] || "fa-solid fa-cube";
  }

  function iconHtml(className) {
    return '<i class="' + escapeAttribute(className) + '" aria-hidden="true"></i>';
  }
  ...
  function openProductDialog(productId) {
    ...
    dom.dialogBody.innerHTML = [
      ...
      '<div><p class="eyebrow">' + escapeHtml(category.title) + " · UPC " + escapeHtml(productUpc(product)) + '</p><h2>' + escapeHtml(product.name) + "</h2></div>",
      '<div class="product-meta"><span class="brand-chip"><i class="fa-solid fa-tag" aria-hidden="true"></i>' + escapeHtml(product.brand) + '</span><span class="stock-chip"><i class="fa-solid fa-boxes-stacked" aria-hidden="true"></i>' + escapeHtml(product.stock) + ' disp.</span><span class="status-chip ' + compat.level + '">' + escapeHtml(compat.shortLabel) + "</span></div>",
      ...
      '<div class="product-footer"><div class="price-block"><span>' + escapeHtml(priceSourceLabel) + '</span><div class="price">' + currency.format(product.price) + '</div></div><button class="product-action" type="button" data-dialog-select="' + escapeHtml(product.id) + '">' + iconHtml(isSelected(product.id) ? "fa-solid fa-xmark" : "fa-solid fa-plus") + (isSelected(product.id) ? "Quitar" : "Agregar") + "</button></div>",
      "</div>",
      "</div>"
    ].join("");
  ...
  function generateQuoteHtml() {
    ...
    var rows = ctx.entries.map(function (entry) {
      return [
        "<tr>",
        "<td>" + escapeHtml(productUpc(entry.product)) + "</td>",
    ...
    var planRows = ctx.plans.map(function (plan) {
      return [
        "<tr>",
        "<td>Descuento único " + plan.discountRate + "%</td>",
    ...
    return [
      ...
      "<style>",
      "@page{size:A4;margin:14mm}body{font-family:Arial,sans-serif;color:#172026;margin:0;line-height:1.4;background:#fff}.page{max-width:980px;margin:0 auto}.head{display:grid;grid-template-columns:1fr auto;gap:24px;align-items:start;border-bottom:3px solid #172026;padding-bottom:16px}.logo-row{display:flex;align-items:center;gap:14px}.logo{width:74px;height:74px;object-fit:contain}.muted{color:#68737d}.kicker{margin:0 0 4px;text-transform:uppercase;font-size:11px;font-weight:800;color:#68737d;letter-spacing:.03em}h1,h2{margin:0}h1{font-size:28px}h2{font-size:15px}.quote-id{font-size:16px;font-weight:800}.box{border:1px solid #d7dde2;border-radius:8px;padding:12px;margin-top:14px;break-inside:avoid}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.summary{display:grid;grid-template-columns:1fr 1fr 1fr 1.2fr;gap:10px}.summary .box{margin-top:14px}.summary strong{display:block;font-size:16px;margin-top:3px}.summary .final{background:#eef7f5;border-color:#0f8b7d}table{width:100%;border-collapse:collapse;margin-top:14px;break-inside:auto}thead{display:table-header-group}tr{break-inside:avoid}th,td{border-bottom:1px solid #d7dde2;padding:8px;text-align:left;font-size:11.5px;vertical-align:top}th{background:#eef3f5;color:#33404a;text-transform:uppercase;font-size:10px}.right{text-align:right}.notes{white-space:pre-wrap}.free{color:#177245;font-weight:800}.terms{font-size:11px;color:#68737d}.badge{display:inline-block;border-radius:999px;padding:4px 8px;background:#eef3f5;font-size:11px;font-weight:700}@media print{.no-print{display:none}.page{max-width:none}}",
      "</style></head><body>",
      '<div class="page"><div class="head"><div class="logo-row"><img class="logo" src="logo-ds.svg" alt="Logo"><div><p class="kicker">Cotización profesional de combo PC</p><h1>' + escapeHtml(state.quote.id) + '</h1><p class="muted">Inventario: ' + escapeHtml(catalog.sourceFile) + " · Base comercial: " + escapeHtml(priceSourceLabel) + "</p></div></div>",
      "<div><p class=\"quote-id\">Estado: " + escapeHtml(ctx.analysis.label) + "</p><strong>Fecha:</strong> " + escapeHtml(formatDate(ctx.created)) + "<br><strong>Válida hasta:</strong> " + escapeHtml(formatDate(ctx.validUntil)) + "<br><span class=\"badge\">Ensamble gratis</span></div></div>",
      '<div class="grid"><div class="box"><h2>Cliente</h2><p>' + (customer || "N/D") + '</p></div><div class="box"><h2>Condiciones</h2><p class="terms">Precios sujetos a disponibilidad, confirmación final de ventas y vigencia indicada. Compatibilidad validada por socket, DDR, gabinete y fuente.</p></div></div>',
      "<table><thead><tr><th>UPC</th><th>Categoría</th><th>Producto</th><th class=\"right\">Cant.</th><th class=\"right\">" + escapeHtml(priceSourceLabel) + "</th><th class=\"right\">Total</th></tr></thead><tbody>",
      rows || '<tr><td colspan="6">Sin productos seleccionados.</td></tr>',
      "</tbody></table>",
      '<div class="summary"><div class="box"><span class="muted">Subtotal</span><strong>' + currency.format(ctx.totals.subtotal) + '</strong></div><div class="box"><span class="muted">Descuento ' + customerDiscountRate + '%</span><strong>-' + currency.format(ctx.totals.discount) + '</strong></div><div class="box"><span class="muted">Ensamble</span><strong class="free">Gratis</strong></div><div class="box final"><span class="muted">Total final</span><strong>' + currency.format(ctx.totals.total) + "</strong></div></div>",
      '<div class="box"><h2>Resumen comercial</h2><table><thead><tr><th>Concepto</th><th class="right">Ahorro</th><th class="right">Total</th></tr></thead><tbody>' + planRows + "</tbody></table></div>",
  ...
  function generateExcelBlob() {
    ...
    var rows = [
      ...
      ["Fecha", formatDate(ctx.created)],
      ["Válida hasta", formatDate(ctx.validUntil)],
      ["Estado técnico", ctx.analysis.label],
      ["Cliente", state.quote.clientName || "Cliente sin nombre"],
      ["Contacto", state.quote.clientContact || ""],
      ["Correo", state.quote.clientEmail || ""],
      [],
      ["Categoría", "UPC", "Producto", "Marca", "Cantidad", priceSourceLabel, "Total", "Stock"]
    ];

    ctx.entries.forEach(function (entry) {
      rows.push([
        categoryById(entry.product.category).title,
        productUpc(entry.product),
        entry.product.name,
        entry.product.brand || "",
        entry.qty,
        roundMoney(entry.product.price),
    ...
    rows.push([]);
    rows.push(["Subtotal " + priceSourceLabel, ctx.totals.subtotal]);
    rows.push(["Descuento " + customerDiscountRate + "%", -ctx.totals.discount]);
    rows.push(["Ensamble", "Gratis"]);
    rows.push(["Total final", ctx.totals.total]);
    rows.push([]);
    rows.push(["Validación técnica"]);
    if (ctx.analysis.messages.length) {
      ctx.analysis.messages.forEach(function (message) {
        rows.push([message.severity.toUpperCase(), message.text]);
      });
    } else {
      rows.push(["OK", "Sin observaciones."]);
    }
    if (state.quote.notes) {
      rows.push([]);
      rows.push(["Notas", state.quote.notes]);
    }
    ...
    var files = {
      ...
      "xl/workbook.xml": "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><workbook xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\"><sheets><sheet name=\"Cotizacion\" sheetId=\"1\" r:id=\"rId1\"/></sheets></workbook>",
  ...
  function generateWorksheetXml(rows) {
    var sheetRows = rows.map(function (row, rowIndex) {
      var cells = row.map(function (value, columnIndex) {
        return excelCell(value, columnIndex, rowIndex);
      }).join("");
      return '<row r="' + (rowIndex + 1) + '">' + cells + "</row>";
    }).join("");
    return "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><cols><col min=\"1\" max=\"1\" width=\"22\" customWidth=\"1\"/><col min=\"2\" max=\"2\" width=\"20\" customWidth=\"1\"/><col min=\"3\" max=\"3\" width=\"54\" customWidth=\"1\"/><col min=\"4\" max=\"4\" width=\"18\" customWidth=\"1\"/><col min=\"5\" max=\"8\" width=\"14\" customWidth=\"1\"/></cols><sheetData>" + sheetRows + "</sheetData></worksheet>";
  }

  function excelCell(value, columnIndex, rowIndex) {
    var ref = columnName(columnIndex) + (rowIndex + 1);
  ...
  function downloadQuotePdf() {
    if (!validateQuoteForDelivery("generar el PDF")) {
      return;
    }
    openQuotePrintWindow();
    toast("Cotización lista para guardar como PDF.");
  }

  function downloadExcel() {
    if (!validateQuoteForDelivery("descargar Excel")) {
      return;
    }
    downloadBlob(generateExcelBlob(), filename("xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    toast("Excel descargado.");
  }

  function validateQuoteForDelivery(action) {
    var ctx = quoteContext();
    var errors = [];
    ...
    if (errors.length) {
      toast("No se puede " + action + ": " + errors[0]);
      return false;
    }
    if (!state.quote.clientName && !state.quote.clientContact) {
      toast("Sugerencia: agrega cliente o contacto antes de enviar.");
    }
    return true;
  }

  function printQuote() {
    if (!validateQuoteForDelivery("imprimir la cotización")) {
      return;
    }
    openQuotePrintWindow();
  }
  ...
  function zipFiles(files) {
    ...
    names.forEach(function (name) {
      ...
      var center = new Uint8Array(46);
      var centerView = new DataView(center.buffer);
      ...
      centerView.setUint16(12, stamp.time, true);
      centerView.setUint16(14, stamp.date, true);
    ...
    return concatBytes(chunks.concat(central, [end]));
  }
  ...
  function crc32(bytes) {
    if (!crcTable) {
      crcTable = Array.from({ length: 256 }, function (_, n) {
        var c = n;
        for (var k = 0; k < 8; k += 1) {
  ...
  function sendWhatsappQuote() {
    if (!validateQuoteForDelivery("enviar por WhatsApp")) {
      return;
    }
  ...
  function generateWhatsappText() {
    var ctx = quoteContext();
    var lines = [
      "Hola, comparto la cotización solicitada:",
      "",
      "*" + state.quote.id + " | Combo PC*",
      "Cliente: " + (state.quote.clientName || "por confirmar"),
      state.quote.clientContact ? "Contacto: " + state.quote.clientContact : "",
      state.quote.clientEmail ? "Correo: " + state.quote.clientEmail : "",
      "Estado técnico: " + ctx.analysis.label,
      "Vigencia: " + formatDate(ctx.validUntil),
      "Base de precio: " + priceSourceLabel,
      "",
    ...
    if (!ctx.entries.length) {
      ...
      ctx.entries.forEach(function (entry, index) {
        lines.push((index + 1) + ". " + categoryById(entry.product.category).title);
        lines.push("   Producto: " + entry.product.name);
        lines.push("   UPC: " + productUpc(entry.product));
        lines.push("   Cantidad: " + entry.qty);
        lines.push("   Precio unitario: " + currency.format(entry.product.price));
        lines.push("   Subtotal: " + currency.format(entry.product.price * entry.qty));
      });
    }

    lines.push("");
    lines.push("*Resumen comercial*");
    lines.push("Subtotal " + priceSourceLabel + ": " + currency.format(ctx.totals.subtotal));
    lines.push("Descuento aplicado (" + customerDiscountRate + "%): -" + currency.format(ctx.totals.discount));
    lines.push("Ensamble: Gratis");
    lines.push("*Total final: " + currency.format(ctx.totals.total) + "*");

    if (ctx.analysis.messages.length) {
      lines.push("");
      lines.push("*Validación técnica*");
      ctx.analysis.messages.forEach(function (message) {
        lines.push("- " + message.severity.toUpperCase() + ": " + message.text);
      });
    }
    ...
    if (state.quote.notes) {
      lines.push("");
      lines.push("*Notas*");
      lines.push(state.quote.notes);
    }

    lines.push("");
    lines.push("Quedo atento para confirmar disponibilidad y proceder con el armado.");
  ...
  function runSmokeTests() {
    ...
    assert(products.every(function (product) { return product.code && productUpc(product) !== "N/D" && product.price > 0 && product.stock > 0; }), "todos los productos tienen UPC, Precio 4 y stock");
    ...
    assert(generateQuoteHtml().indexOf("Cotización profesional de combo PC") >= 0, "cotización imprimible se genera");
    assert(generateExcelBlob() instanceof Blob, "Excel XLSX se genera");
    assert(generateWhatsappText().indexOf("Resumen comercial") >= 0, "mensaje de WhatsApp se genera claro");
    assert(calculateTotals().discountRate === customerDiscountRate && calculateTotals().total < calculateTotals(0).total, "descuento único de 15% reduce el total");
  ...
  window.PCBuilderApp = {
    ...
    generateExcelBlob: generateExcelBlob,
